                                                          ...
                                                          float aZbot )
{
    ...
    float inner = aRing->GetInnerRadius();
    float outer = aRing->GetOuterRadius();
...
void C3D_RENDER_OGL_LEGACY::add_object_to_triangle_layer( const CROUNDSEGMENT2D * aSeg,
                                                          ...
                                                          float aZbot )
{
    SFVEC2F leftStart   = aSeg->GetLeftStar();
    SFVEC2F leftEnd     = aSeg->GetLeftEnd();
    SFVEC2F leftDir     = aSeg->GetLeftDir();

    SFVEC2F rightStart  = aSeg->GetRightStar();
    SFVEC2F rightEnd    = aSeg->GetRightEnd();
    SFVEC2F rightDir    = aSeg->GetRightDir();
    float   radius      = aSeg->GetRadius();

    SFVEC2F start       = aSeg->GetStart();
    SFVEC2F end         = aSeg->GetEnd();

    float texture_factor = (12.0f / (float)SIZE_OF_CIRCLE_TEXTURE) + 1.0f;
    float texture_factorF= ( 6.0f / (float)SIZE_OF_CIRCLE_TEXTURE) + 1.0f;
...
void C3D_RENDER_OGL_LEGACY::reload( REPORTER *aStatusTextReporter )
{
    ...
    printf( "  Reload board:             %.3f ms\n",
            (float)( stats_endReloadTime        - stats_startReloadTime        ) /
            1000.0f );
    printf( "  Loading to openGL:        %.3f ms\n",
            (float)( stats_end_OpenGL_Load_Time - stats_start_OpenGL_Load_Time ) /
            1000.0f );
    printf( "  Loading 3D models:        %.3f ms\n",
            (float)( stats_end_models_Load_Time - stats_start_models_Load_Time ) /
            1000.0f );
