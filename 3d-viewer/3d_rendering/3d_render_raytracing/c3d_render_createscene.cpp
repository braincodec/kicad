...
void C3D_RENDER_RAYTRACING::setupMaterials()
{
    ...
    m_materials.m_Paste = CBLINN_PHONG_MATERIAL(
                ...
                SFVEC3F( 0.0f, 0.0f, 0.0f ),            // emissive
                (SFVEC3F)m_settings.m_SolderPasteColor *
                (SFVEC3F)m_settings.m_SolderPasteColor, // specular
                0.10f * 128.0f,                         // shiness
                0.0f,                                   // transparency
                0.0f );

    m_materials.m_SilkS = CBLINN_PHONG_MATERIAL(
                SFVEC3F( 0.11f ),                       // ambient
                SFVEC3F( 0.0f, 0.0f, 0.0f ),            // emissive
                glm::clamp( ((SFVEC3F)(1.0f) -
...
void C3D_RENDER_RAYTRACING::reload( REPORTER *aStatusTextReporter )
{
    ...
    m_lights.Add( new CDIRECTIONALLIGHT( SphericalToCartesian( glm::pi<float>() * 0.03f,
                                                               glm::pi<float>() * 0.25f ), SFVEC3F( light_directional_intensity_top ) ) );

    m_lights.Add( new CDIRECTIONALLIGHT( SphericalToCartesian( glm::pi<float>() * 0.97f,
                                                               glm::pi<float>() * 1.25f ), SFVEC3F( light_directional_intensity_top ) ) );
        ...
        if( m_settings.GetFlag( FL_SHOW_BOARD_BODY ) )
        {
                 ...
                 ++object2d_iterator )
            {
                ...
                if( !m_settings.GetThroughHole_Outer().GetList().empty() )
                {
                    ...
                    m_settings.GetThroughHole_Outer().GetListObjectsIntersects( object2d_A->GetBBox(),
                                                                                intersectionList );
                ...
                if( object2d_B == CSGITEM_EMPTY )
                {
                    ...
                    objPtr->SetColor( (SFVEC3F)m_settings.m_BoardBodyColor );
        ...
        (m_outlineBoard2dObjects->GetList().size() >= 1) )
    {
             ...
             ++ii )
        {
            ...
            const CBVHCONTAINER2D *containerLayer2d = static_cast<const CBVHCONTAINER2D *>(ii->second);
    ...
    if( m_settings.GetFlag( FL_RENDER_RAYTRACING_BACKFLOOR ) )
    {
        ...
        if( boardBBox.IsInitialized() )
        {
            ...
            if( m_object_container.GetList().size() > 0 )
            {
                ...
                newTriangle1->SetColor( (SFVEC3F)m_settings.m_BgColorTop );
                newTriangle2->SetColor( (SFVEC3F)m_settings.m_BgColorTop );
            }
        }
    }
    ...
    if( aStatusTextReporter )
    {
        ...
        aStatusTextReporter->Report( wxString::Format( _( "Reload time %.3f s" ), calculation_time ) );
    }
}
...
void C3D_RENDER_RAYTRACING::insert3DViaHole( const VIA* aVia )
{
    ...
    if( m_settings.GetFlag( FL_USE_REALISTIC_MODE ) )
        objPtr->SetColor( (SFVEC3F)m_settings.m_CopperColor );
    else
        objPtr->SetColor( m_settings.GetItemColor( VIAS_VISIBLE + aVia->GetViaType() ) );
...
void C3D_RENDER_RAYTRACING::insert3DPadHole( const D_PAD* aPad )
{
    ...
    else    // Oblong hole
    {
        ...
        CROUNDSEGMENT2D *outterSeg = new CROUNDSEGMENT2D(
                                    ...
                                    SFVEC2F( end.x   * m_settings.BiuTo3Dunits(),
                                            -end.y * m_settings.BiuTo3Dunits() ),
                                    ( width +  m_settings.GetCopperThicknessBIU() * 2 ) * m_settings.BiuTo3Dunits(),
                                    *aPad );
...
void C3D_RENDER_RAYTRACING::load_3D_models()
{
         ...
         module = module->Next() )
    {
            ...
            m_settings.ShouldModuleBeDisplayed( (MODULE_ATTR_T)module->GetAttributes() ) )
        {
            ...
            {
                // get it from cache
                const S3DMODEL *modelPtr = m_settings.Get3DCacheManager()->GetModel( sM->m_Filename );
                        ...
                        ( idx2 < mesh.m_VertexSize ) )
                    {
                        ...
                        if( mesh.m_Color == NULL )
                        {
                            ...
                            if( m_settings.MaterialModeGet() == MATERIAL_MODE_CAD_MODE )
                                newTriangle->SetColor( MaterialDiffuseToColorCAD( diffuseColor ) );
                            else
                                newTriangle->SetColor( diffuseColor );
                        }
                        else
                        {
                            if( m_settings.MaterialModeGet() == MATERIAL_MODE_CAD_MODE )
                                newTriangle->SetColor(
                                    MaterialDiffuseToColorCAD( mesh.m_Color[idx0] ),
                                    MaterialDiffuseToColorCAD( mesh.m_Color[idx1] ),
                                    MaterialDiffuseToColorCAD( mesh.m_Color[idx2] ) );
                            else
                                newTriangle->SetColor( mesh.m_Color[idx0],
                                                       mesh.m_Color[idx1],
                                                       mesh.m_Color[idx2] );
                        }
                    }
