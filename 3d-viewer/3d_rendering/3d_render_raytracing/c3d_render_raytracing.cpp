...
bool C3D_RENDER_RAYTRACING::Redraw( bool aIsMoving, REPORTER *aStatusTextReporter )
{
    ...
    if( requestRedraw || aIsMoving || was_camera_changed )
        m_rt_render_state = RT_RENDER_STATE_MAX; // Set to an invalid state, so it will restart again latter
                    ...
                    if( hitPacket[i].m_hitresult == true )
                    {
                            ...
                            (nodex0y0 == nodex1y1) )
                        {
                            ...
                            */
                            ...
                            // Option 2
                            // Trace again with the same node, then if miss just give the same color as before
                            //if( m_accelerator->Intersect( rayAA, hitAA, nodex0y0 ) )
                            //    hitColorAA[i] += shadeHit( bgColor[y], rayAA, hitAA, false, 0 );
                            //else
        ...
        for( unsigned int y = 0, i = 0; y < (RAYPACKET_DIM - 1); ++y )
        {
            for( unsigned int x = 0; x < (RAYPACKET_DIM - 1); ++x, ++i )
            {
                    ...
                    hitPacket[ idx1y1 ].m_hitresult )
                {
                    ...
                    // Average
                    /*
                    const float gray_idx0y0 = (hitColor[idx0y0].r + hitColor[idx0y0].g + hitColor[idx0y0].b ) / 3.0f;
                    const float gray_idx1y0 = (hitColor[idx1y0].r + hitColor[idx1y0].g + hitColor[idx1y0].b ) / 3.0f;
                    const float gray_idx0y1 = (hitColor[idx0y1].r + hitColor[idx0y1].g + hitColor[idx0y1].b ) / 3.0f;
                    const float gray_idx1y1 = (hitColor[idx1y1].r + hitColor[idx1y1].g + hitColor[idx1y1].b ) / 3.0f;
                    */
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        const unsigned x0y0 = (x - 1) + (RAYPACKET_DIM - 1) * (y - 0);
                                        const unsigned x1y0 = (x - 0) + (RAYPACKET_DIM - 1) * (y - 0);
                                    ...
                                    {
                                        if( ( y == (RAYPACKET_DIM - 1) ) && ( x < (RAYPACKET_DIM - 1) ) )
                                        {
                                            const unsigned x0y0 = (x - 1) + (RAYPACKET_DIM - 1) * (y - 1);
                                            const unsigned x1y0 = (x - 0) + (RAYPACKET_DIM - 1) * (y - 1);
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                const unsigned x0y0 = (x - 0) + (RAYPACKET_DIM - 1) * (y - 1);
                                                const unsigned x0y1 = (x - 0) + (RAYPACKET_DIM - 1) * (y - 0);
            ...
            if( m_settings.GetFlag( FL_RENDER_RAYTRACING_POST_PROCESSING ) )
            {
                if( hitPacket[i].m_hitresult == true )
                    m_postshader_ssao.SetPixelData( blockPos.x + x, blockPos.y + y,
                                                    ...
                                                    hColor,
                                                    blockPacket.m_ray[i].at( hitPacket[i].m_HitInfo.m_tHit ),
                                                                ...
                                                                REPORTER *aStatusTextReporter )
{
    ...
    if( m_settings.GetFlag( FL_RENDER_RAYTRACING_POST_PROCESSING ) )
    {
        ...
        for( signed int y = 0; y < (int)m_realBufferSize.y; ++y )
        {
            ...
            SFVEC3F *ptrShaderY0 = &m_shaderBuffer[ glm::max((int)y - 2, 0) * m_realBufferSize.x ];
            SFVEC3F *ptrShaderY1 = &m_shaderBuffer[ glm::max((int)y - 1, 0) * m_realBufferSize.x ];
            SFVEC3F *ptrShaderY2 = &m_shaderBuffer[ y * m_realBufferSize.x ];
            SFVEC3F *ptrShaderY3 = &m_shaderBuffer[ glm::min((int)y + 1, (int)(m_realBufferSize.y - 1)) * m_realBufferSize.x ];
            SFVEC3F *ptrShaderY4 = &m_shaderBuffer[ glm::min((int)y + 2, (int)(m_realBufferSize.y - 1)) * m_realBufferSize.x ];
...
#else
                // Debug code
                //const SFVEC3F shadedColor =  SFVEC3F(1.0f) - m_shaderBuffer[ y * m_realBufferSize.x + x];
            ...
            for( unsigned int x = 0; x < (RAYPACKET_DIM - 1); ++x )
            {
                ...
                if( y == 0 )
                {
                        ...
                        (hitPacket[ iLT ].m_HitInfo.pHitObject == hitPacket[ iRT ].m_HitInfo.pHitObject) )
                    {
                        ...
                        hitInfoLRT.m_tHit = ( hitPacket[ iLT ].m_HitInfo.m_tHit + hitPacket[ iRT ].m_HitInfo.m_tHit ) * 0.5f;
                        hitInfoLRT.m_HitNormal = glm::normalize( ( hitPacket[ iLT ].m_HitInfo.m_HitNormal + hitPacket[ iRT ].m_HitInfo.m_HitNormal ) * 0.5f );
                ...
                if( x == 0 )
                {
                          ...
                          hitPacket[ iLB ].m_HitInfo.pHitObject ) )
                    {
                        ...
                        hitInfoLTB.m_HitNormal = glm::normalize( ( hitPacket[ iLT ].m_HitInfo.m_HitNormal +
                      ...
                      hitPacket[ iRB ].m_HitInfo.pHitObject ) )
                {
                    ...
                    hitInfoRTB.m_HitNormal = glm::normalize( ( hitPacket[ iRT ].m_HitInfo.m_HitNormal +
                ...
                if( hitPacket[ iLB ].m_hitresult &&
                    hitPacket[ iRB ].m_hitresult &&
                    (hitPacket[ iLB ].m_HitInfo.pHitObject == hitPacket[ iRB ].m_HitInfo.pHitObject) )
                {
                    ...
                    hitInfoLRB.m_HitNormal = glm::normalize( ( hitPacket[ iLB ].m_HitInfo.m_HitNormal +
                                                               hitPacket[ iRB ].m_HitInfo.m_HitNormal ) * 0.5f );
                                         ...
                                         unsigned int aRecursiveLevel ) const
{
         ...
         ++ii )
    {
        ...
        /*
        if( (!m_isPreview) &&
            m_settings.GetFlag( FL_RENDER_RAYTRACING_POST_PROCESSING ) )        // Little hack to make randomness to the shading and shadows
            vectorToLight = glm::normalize( vectorToLight + UniformRandomHemisphereDirection() * 0.1f );
         */
    ...
    if( !m_isPreview )
    {
            ...
            m_settings.GetFlag( FL_RENDER_RAYTRACING_REFLECTIONS ) )
        {
            ...
            for( unsigned int i = 0; i < reflection_number_of_samples; ++i )
            {
                ...
                const SFVEC3F random_reflectVector = glm::normalize( reflectVector +
