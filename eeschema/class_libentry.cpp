...
LIB_PART LIB_PART::m_dummy( wxEmptyString );
...
/* static */ LIB_PART* LIB_PART::GetDummy()
{
    if( !m_dummy.drawings.size() )
    {
        std::auto_ptr<LIB_RECTANGLE> square( new LIB_RECTANGLE( &m_dummy ) );
        std::auto_ptr<LIB_TEXT> text( new LIB_TEXT( &m_dummy ) );
        ...
        m_dummy.AddDrawItem( square.release() );
        m_dummy.AddDrawItem( text.release() );
    }
